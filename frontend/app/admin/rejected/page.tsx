...
import { fetchAdminRejectedUsers, type AdminUser } from "@/lib/api";
...
export default function RejectedUsersPage() {
  ...
  async function loadUsers() {
    try {
      const data = await fetchAdminRejectedUsers();
      setUsers(Array.isArray(data) ? data : []);
    } catch (error: any) {
      console.error("Failed to fetch rejected users:", error);
      toast.error(error?.message || "Failed to load rejected users.");
    } finally {
      setLoading(false);
  ...
  if (loading) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-neutral-500 text-sm">Loading rejected registrations...</p>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="px-3 py-1 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-semibold">
              {users.length} Rejected
            </span>
          </div>
          <p className="text-neutral-400 text-sm mt-1.5">
            Audit history of denied registrations with detailed rejection rationale.
          </p>
        </div>
      ...
      {users.length === 0 ? (
        <div className="rounded-3xl border border-neutral-900 bg-[#111111] p-12 text-center shadow-lg">
          ...
          <p className="text-neutral-500 text-sm mt-2 max-w-md mx-auto">
            Accounts denied due to fraudulent, incomplete, or invalid documentation will be archived here.
          </p>
        </div>
      ) : (
