    ...
    >
      ...
      <div className="w-full flex flex-col items-center">
        ...
        >
          <Link
            href="/"
            title="LogiTrack Delivery Portal"
        ...
        {/* Navigation Items */}
        <nav className="space-y-2 w-full flex flex-col items-center" aria-label="Delivery Portal Navigation">
          {navItems.map((item) => {
        ...
        {/* Shift Active Capsule */}
        {!collapsed ? (
          <div className="w-full mt-6 rounded-2xl border border-[#2A2B30]/80 bg-[#1A1B1E]/70 p-4 backdrop-blur-sm shadow-sm transition-all">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <span className="flex h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                <span className="text-[11px] uppercase font-semibold tracking-wider text-[#F4F4F5]">
                  Shift Active
                </span>
              </div>
              <Radio size={13} className="text-[#F97316] animate-pulse" />
            </div>
            <p className="text-[11px] text-[#A1A1AA] mt-2">
              Telemetry: <span className="text-[#FDBA74] font-medium">GPS Engine Live</span>
            </p>
          </div>
        ) : (
          <div
            title="Shift Status: Active • GPS Telemetry Live"
            className="mt-4 flex h-11 w-11 mx-auto items-center justify-center rounded-2xl border border-[#2A2B30] bg-[#1A1B1E] text-[#F97316] relative cursor-help"
          >
